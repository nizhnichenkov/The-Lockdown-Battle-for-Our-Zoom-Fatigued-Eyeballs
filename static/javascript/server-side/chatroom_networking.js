...
function startCamera() {
    // console.log('startCamera(): chatroom_networking.js');
    ...
    // console.log("startCamera(): Peer list: \n", _peer_list)
}
...
function handleTrackEvent(event, peer_id) {
    ...
    if (event.streams) {
        ...
        // increment count
        handleTrackEventCount++;

        // enable panning and vad when the video stream is added (for the 2nd time)
        // to the video element
        if (handleTrackEventCount === 2) {
            // specify that the video of the remote is not resized
            isVideoResized[peer_id] = false;

            //    enable panning and vad here
            enableFeatures(peer_id);

            // enable dragging of the div containing the name tag, video and visualiser elements
            dragElement(document.getElementById(`div_vid_${peer_id}`));

            // enable resizing by double click
            resizeElement(document.getElementById(`div_vid_${peer_id}`));

            //    bring count back to 0
            handleTrackEventCount = 0;
        }
    }
}